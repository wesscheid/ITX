...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSave = async (platform: 'youtube' | 'instagram') => {
    const cookies = platform === 'youtube' ? youtubeCookies : instagramCookies;
    if (!cookies.trim()) {
    ...
    try {
      ...
      setStatus({ type: 'success', message: `${platform === 'youtube' ? 'YouTube' : 'Instagram'} cookies updated successfully!` });
      if (platform === 'youtube') setYoutubeCookies('');
      else setInstagramCookies('');
